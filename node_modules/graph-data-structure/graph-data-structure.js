...
function Graph(serialized) {
    ...
    function addEdge(u, v, weight, givenBuses) {
        addNode(u);
        addNode(v);
        adjacent(u).push(addBus(v));
        if (weight !== undefined) {
            setEdgeWeight(u, v, weight);
        }
        adjacent(u).get(v).push(v);
        for(let x of givenBuses){
            adjacent(u).get(v).push(x);
        }
        return graph;
    }

    function addBus(node){
        return buses[node] || [];
    }
